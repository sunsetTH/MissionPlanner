...
namespace MissionPlanner
{
    public class MAVLinkInterface : MAVLink, IDisposable
    {
        ...
        public void Open(bool getparams, bool skipconnectedcheck = false)
        {
            ...
            //frmProgressReporter.UpdateProgressAndStatus(-1, "系统连接");
        ...
        private void OpenBg(object PRsender, bool getparams, ProgressWorkerEventArgs progressWorkerEventArgs)
        {
            ...
            //frmProgressReporter.UpdateProgressAndStatus(-1, "系统正在连接");
            giveComport = true;
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (DateTime.Now > deadline)
                    {
                        ...
                        {
                            progressWorkerEventArgs.ErrorMessage = "未接收到有效数据";
                            throw new Exception(@"不能建立有效连接\n
请检查一下项目：
1. 连接端口、波特率设置正确
2.硬件连接正常
3.已经插入SD卡"  );
        ...
        */
        ...
        public void getParamList()
        {
            ...
            //frmProgressReporter.UpdateProgressAndStatus(-1, "系统配置参数获取进度");
            ThemeManager.ApplyThemeTo(frmProgressReporter);
        ...
        public float GetParam(byte sysid, byte compid, string name = "", short index = -1, bool requireresponce = true)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    throw new TimeoutException("获取参数超时！");
                }
